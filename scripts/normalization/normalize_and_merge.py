...
def load_and_normalize_dataset_streaming(
    ...
    try:
        ...
        if TQDM_AVAILABLE:
            ...
            iterator = tqdm(raw_records_list, desc=f"Normalizing {dataset_name}", unit="rec")
        else:
            raw_records_list = list(raw_records)
            total_raw = len(raw_records_list)
        ...
        for idx, record in enumerate(iterator):
            try:
                ...
                unified_record = map_to_unified_schema(
                    ...
                    index=idx,
                    field_mapping=None,  # Auto-detect fields
                    source_file=str(dataset_path)
                )
                
                # Optional validation (can be slow for large datasets)
                if enable_validation:
                    is_valid, validation_errors = validate_record(unified_record, use_jsonschema=False)
        ...
        logger.info(f"  ✅ Normalized {len(records):,} records from {dataset_name} in {elapsed:.2f}s")
        
        if skipped > 0:
            logger.warning(f"  ⚠️  Skipped {skipped:,} invalid/error records from {dataset_name}")
        ...
        stats = {
            ...
            'unique_projects': len(set(r.get('project') for r in records if r.get('project'))),
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_dataset = {
            executor.submit(
                ...
                name, path, max_records, enable_validation, log_dir
            ): name
        ...
        for future in as_completed(future_to_dataset):
            ...
            try:
                ...
                if records:
                    ...
                    logger.info(f"  ✅ {dataset_name}: {len(records):,} records normalized")
                else:
...
def print_summary_table(dataset_stats_list: List[Dict[str, Any]], unified_stats: Dict[str, Any]):
    """
    ...
    if RICH_AVAILABLE and console:
        ...
        for stats in sorted(dataset_stats_list, key=lambda x: x['total_records'], reverse=True):
            table.add_row(
                stats['dataset'],
        ...
        print("\n" + "=" * 100)
        print(f"{'Dataset':<20} {'Records':>10} {'Lang':>6} {'Vulnerable':>12} {'Safe':>10} {'CWEs':>6} {'Time(s)':>10}")
        print("=" * 100)
        
        for stats in sorted(dataset_stats_list, key=lambda x: x['total_records'], reverse=True):
            print(f"{stats['dataset']:<20} {stats['total_records']:>10,} "
                  f"{len(stats['languages']):>6} {stats['vulnerable']:>12,} "
                  f"{stats['safe']:>10,} {stats['unique_cwes']:>6} "
                  f"{stats.get('processing_time_s', 0):>10.2f}")
        
        print("=" * 100)
        print(f"{'TOTAL':<20} {unified_stats['total_records']:>10,} "
...
def generate_markdown_report(
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- **Vulnerable:** {unified_stats['vulnerable_records']:,} ({unified_stats.get('vulnerability_ratio', 0):.2%})\n")
        ...
        f.write("| Dataset | Records | Vulnerable | Safe | Languages | CWEs | Projects |\n")
        f.write("|---------|---------|------------|------|-----------|------|----------|\n")
        
        for stats in sorted(dataset_stats_list, key=lambda x: x['total_records'], reverse=True):
            f.write(f"| {stats['dataset']} | {stats['total_records']:,} | "
        ...
        for lang, counts in sorted(lang_dist.items(), key=lambda x: x[1]['total'], reverse=True)[:10]:
            f.write(f"- **{lang}:** {counts['total']:,} records ({counts['vulnerable']:,} vulnerable)\n")
        ...
        if dedup_stats:
            ...
            for dataset, count in sorted(dedup_stats.items(), key=lambda x: x[1], reverse=True):
...
Examples:
  ...
  python normalize_and_merge_v3.py --output my_merged_dataset.jsonl --summary
    ...
    parser.add_argument(
        '--deduplicate',
        action='store_true',
        help='Enable deduplication by code hash'
    )
    parser.add_argument(
        '--no-dedup',
        action='store_true',
    ...
    logger.info(f"✅ Validation: {'ENABLED' if args.validate else 'DISABLED (fast mode)'}")
    ...
    for stats in dataset_stats_list:
        ...
        save_normalized_dataset_streaming(dataset_records, normalized_file, dataset_name)
    ...
    if all_records:
        ...
        generate_markdown_report(unified_stats, dataset_stats_list, dedup_stats, report_file)
        ...
        print(f"  • Vulnerable: {unified_stats['vulnerable_records']:,} ({unified_stats.get('vulnerability_ratio', 0):.2%})")
