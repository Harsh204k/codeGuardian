    ...
    if HAS_LOGURU:
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler(
                    _LOG_DIR
                    / f"phase2_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
                ),
            ],
        )
...
def timed_with_stats(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            func.__timing_stats__["min_time"] = min(
                func.__timing_stats__["min_time"], elapsed
            )
            func.__timing_stats__["max_time"] = max(
                func.__timing_stats__["max_time"], elapsed
            )
...
def profile_memory(func: Callable) -> Callable:
    ...
    if HAS_MEMORY_PROFILER:
        ...
        logger = get_logger(__name__)
        logger.warning(
            f"memory_profiler not available, skipping profiling for {func.__name__}"
        )
        return func
...
class DatasetLogger:
    ...
    def log_progress(self, records_processed: int, total_records: int):
        ...
        self.logger.info(
            f"Progress: {records_processed:,}/{total_records:,} ({percentage:.1f}%)"
        )
...
def generate_performance_report(
    ...
    with open(output_path, "w") as f:
        ...
        for func_name, stats in sorted(
            timing_data.items(), key=lambda x: x[1].get("total_time", 0), reverse=True
        ):
            ...
            f.write(
                f"| {func_name} | {calls} | {total:.3f}s | {avg:.3f}s | {min_t:.3f}s | {max_t:.3f}s |\n"
            )
