...
Examples:
  ...
  python enhanced_cli.py benchmark diversevul.json      # Benchmark mode
    ...
    scan_parser.add_argument('--mode', choices=['rules', 'ml', 'hybrid'], default='hybrid',
    ...
    scan_parser.add_argument('--format', choices=['json', 'sarif', 'text'], default='text',
                           help='Output format (default: text)')
    scan_parser.add_argument('--confidence', type=float, default=0.5,
                           help='ML confidence threshold (0.0-1.0, default: 0.5)')
    scan_parser.add_argument('--no-ml', action='store_true',
    ...
    bench_parser.add_argument('--samples', type=int, default=100,
                            help='Number of samples to test (default: 100)')
    bench_parser.add_argument('--mode', choices=['rules', 'ml', 'hybrid'], default='hybrid',
                            help='Detection mode (default: hybrid)')
    bench_parser.add_argument('--balanced', action='store_true',
    ...
    train_parser.add_argument('--samples', type=int, default=10000,
                            help='Training samples (default: 10000)')
...
def scan_single_file(scanner: HybridVulnerabilityScanner, file_path: str, 
...
def scan_directory(scanner: HybridVulnerabilityScanner, directory: str, 
                  recursive: bool, mode: str, confidence: float) -> List[Dict]:
    ...
    extensions = {'.c', '.cpp', '.cc', '.cxx', '.h', '.hpp', 
                 '.java', '.py', '.php', '.js', '.ts', '.cs'}
    
    if recursive:
        files = [f for f in path.rglob('*') if f.suffix.lower() in extensions and f.is_file()]
    else:
        files = [f for f in path.iterdir() if f.suffix.lower() in extensions and f.is_file()]
...
def format_results(results: List[Dict], output_format: str) -> str:
    ...
    if output_format == 'json':
        ...
        for result in results:
            if result.get('final_assessment') == 'vulnerable':
                sarif_result = {
                    ...
                    "level": "error" if result.get('confidence', 0) > 0.8 else "warning",
                    "message": {"text": f"Potential vulnerability detected (confidence: {result.get('confidence', 0):.2f})"},
                    "locations": [{
                        "physicalLocation": {
                            "artifactLocation": {"uri": result.get('file_path', '')},
                            "region": {"startLine": 1}
                        }
                    }]
                }
        ...
        vulnerable_files = [r for r in results if r.get('final_assessment') == 'vulnerable']
        ...
        if vulnerable_files:
            ...
            for result in vulnerable_files:
                ...
                output.append(f"      Hybrid Score: {result.get('hybrid_score', 0):.2f}")
                
                if result.get('rule_findings'):
                    output.append(f"      Rule Findings: {len(result['rule_findings'])}")
                    for finding in result['rule_findings'][:3]:  # Show top 3
                        output.append(f"        - {finding.get('message', 'N/A')} (Line {finding.get('line', '?')})")
                
                if result.get('ml_analysis'):
                    output.append(f"      ML Analysis: {result['ml_analysis'].get('explanation', 'N/A')}")
        ...
        if error_files:
            ...
            for result in error_files:
                output.append(f"   📄 {result['file_path']}: {result.get('error', 'Unknown error')}")
...
def status_command(args):
    ...
    try:
        ...
        print(f"✅ Hybrid Scanner: Operational (ML {'enabled' if scanner.ml_enabled else 'disabled'})")
    except Exception as e:
...
def main():
    ...
    if args.command == 'scan':
        ...
        if target_path.is_file():
            ...
            results = [scan_single_file(scanner, str(target_path), mode, args.confidence)]
        else:
            print(f"📁 Scanning directory: {target_path}")
            results = scan_directory(scanner, str(target_path), args.recursive, mode, args.confidence)
