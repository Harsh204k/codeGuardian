...
def train_xgboost_with_proper_split():
    ...
    code_samples, labels = load_diversevul_dataset(max_samples=100000)  # Use 100K for faster training
    ...
    logger.info(f"   Train: {len(train_codes):,} samples ({sum(train_labels):,} vulnerable)")
    logger.info(f"   Test:  {len(test_codes):,} samples ({sum(test_labels):,} vulnerable)")
