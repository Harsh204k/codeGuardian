...
class VulnerabilityFeatureExtractor:
    ...
    def _extract_single_features(self, code):
        ...
        features["pointer_operations"] = len(
            re.findall(r"[*&]\w+|\w+\s*->\s*\w+", code)
        )
        features["malloc_free_ratio"] = self._calculate_malloc_free_ratio(code_lower)
        features["array_access"] = len(re.findall(r"\w+\[\w*\]", code))

        # 6. String operations
        features["string_concat"] = (
            code_lower.count("strcat")
            + code_lower.count("sprintf")
            + code_lower.count("+")
        )
        features["string_copy"] = code_lower.count("strcpy") + code_lower.count(
            "strncpy"
        )
        features["string_length_checks"] = code_lower.count(
            "strlen"
        ) + code_lower.count("sizeof")

        # 7. Input validation patterns
        features["input_validation"] = len(
            re.findall(r"\b(validate|check|verify|sanitize)\b", code_lower)
        )
        features["bounds_checks"] = len(
            re.findall(r"\b(bounds|limit|range|size)\b", code_lower)
        )
        features["null_checks"] = (
            code_lower.count("null")
            + code_lower.count("nullptr")
            + code_lower.count("== 0")
        )

        # 8. Error handling
        features["try_catch"] = len(
            re.findall(r"\b(try|catch|except|finally)\b", code_lower)
        )
        features["error_returns"] = len(
            re.findall(r"return\s*(-1|null|error|false)", code_lower)
        )

        # 9. Cryptographic patterns
        features["crypto_operations"] = len(
            re.findall(r"\b(encrypt|decrypt|hash|cipher|key|crypto)\b", code_lower)
        )
        features["random_generation"] = len(
            re.findall(r"\b(random|rand|srand|urandom)\b", code_lower)
        )

        # 10. Network and file operations
        features["network_operations"] = len(
            re.findall(r"\b(socket|connect|send|recv|http|url)\b", code_lower)
        )
        features["file_operations"] = len(
            re.findall(r"\b(fopen|fclose|fread|fwrite|file)\b", code_lower)
        )

        # 11. Language-specific patterns
        features["sql_patterns"] = len(
            re.findall(r"\b(select|insert|update|delete|union|drop)\b", code_lower)
        )
        features["web_patterns"] = len(
            re.findall(r"\b(request|response|session|cookie|header)\b", code_lower)
        )
...
def load_full_dataset(max_samples=100000):
    ...
    with open(dataset_path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            if i % 50000 == 0 and i > 0:
                logger.info(
                    f"   Processed {i:,} lines, found {len(vulnerable):,} vuln, {len(safe):,} safe"
                )
    ...
    logger.info(
        f"✅ Dataset loaded: {len(vulnerable):,} vulnerable, {len(safe):,} safe samples"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"📈 F1-Score: {results['f1_score']:.3f} on {results['dataset_size']:,} samples"
        )
