...
Combines rule-based detection with XGBoost ML semantic ana    def __init__(self, model_dir=None):
        ...
        if model_dir:
            ...
            if split_models:
                ...
                if xgboost_models:
                    self.model_path = max(xgboost_models, key=lambda p: p.stat().st_mtime)
                else:
...
import sys
sys.path.append('.')
from engine.scanner import scan_files, Finding
...
class VulnerabilityFeatureExtractor:
    ...
    def _extract_single_features(self, code):
        ...
        features['bounds_checks'] = len(re.findall(r'(length|size|bounds|check|validate)', code_lower))
        features['string_concat'] = len(re.findall(r'(\+\s*"|strcat|concat)', code_lower))
        features['hardcoded_values'] = len(re.findall(r'(password|secret|key)\s*=\s*["\']', code_lower))
        features['file_operations'] = len(re.findall(r'(fopen|fread|fwrite|file_get|file_put)', code_lower))
        features['network_calls'] = len(re.findall(r'(socket|connect|send|recv|http|url)', code_lower))
        features['privilege_calls'] = len(re.findall(r'(admin|root|sudo|privilege|permission)', code_lower))
        ...
        feature_names.extend([
            ...
            'bounds_checks', 'string_concat', 'hardcoded_values',
            'file_operations', 'network_calls', 'privilege_calls'
        ])
...
class MLVulnerabilityDetector:
    ...
    def load_model(self):
        ...
        try:
            if not self.model_path.exists():
                logger.warning(f"ML model not found at {self.model_path}. ML features disabled.")
                return False
    ...
    def predict_vulnerability(self, code: str, confidence_threshold: float = 0.5) -> Tuple[bool, float, str]:
        """
        ...
        try:
            ...
            vuln_prob = self.model.predict_proba(features_array)[0][1]  # Probability of vulnerability
    ...
    def _generate_explanation(self, confidence: float, is_vulnerable: bool) -> str:
        ...
        if is_vulnerable:
            if confidence > 0.9:
                ...
                return f"Likely vulnerability detected (ML confidence: {confidence:.2%})"
            else:
...
class HybridVulnerabilityScanner:
    ...
    def __init__(self, enable_ml: bool = True):
        # Load rules
        try:
            self.rules = load_rules("java,python,cpp,csharp,php")  # Load all language rules
        ...
        if self.ml_detector:
            ...
            logger.info(f"ML enhancement: {'enabled' if self.ml_enabled else 'disabled'}")
    ...
    def scan_file(self, file_path: str, mode: str = "hybrid") -> Dict:
        ...
        try:
            ...
            if mode == 'hybrid':
                results.update(self._combine_analyses(results['rule_findings'], results['ml_analysis']))
            elif mode == 'rules':
    ...
    def _run_rule_scanner(self, file_path: str) -> List[Dict]:
        ...
        try:
            ...
            language = lang_map.get(file_ext, 'cpp')  # Default to cpp for unknown extensions
            
            # Create the file input format expected by scan_files
            file_pairs = [(language, Path(file_path))]
            
            # Use existing scanner logic
            findings = scan_files(file_pairs, self.rules, profile="balanced", appname="HybridScan")
    ...
    def _run_ml_analysis(self, code_content: str) -> Optional[Dict]:
        ...
        try:
            is_vulnerable, confidence, explanation = self.ml_detector.predict_vulnerability(code_content)
    ...
    def _combine_analyses(self, rule_findings: List[Dict], ml_analysis: Optional[Dict]) -> Dict:
        ...
        if ml_analysis:
            ...
            confidence = max(rule_score, ml_score)  # Higher confidence from either method
        else:
    ...
    def _assess_rule_findings(self, rule_findings: List[Dict]) -> Dict:
        ...
        rule_score = min(len([f for f in rule_findings if f['severity'].lower() in ['high', 'critical']]) * 0.3, 1.0)
    ...
    def _assess_ml_findings(self, ml_analysis: Optional[Dict]) -> Dict:
        ...
        return {
            ...
            'final_assessment': 'vulnerable' if ml_analysis['is_vulnerable'] else 'safe',
...
def demo_hybrid_scanner():
    ...
    test_files = [
        "balanced_test_files/vuln_001.c",
        "balanced_test_files/safe_001.c"
    ]
