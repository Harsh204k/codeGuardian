...
class RuleEngine:
    ...
    def compute_overall_confidence(self, findings: List[Dict[str, Any]]) -> float:
        """
        Compute overall confidence score from multiple findings.
        ...
        for finding in findings:
            # Get confidence (convert string to float if needed)
    ...
    def aggregate_findings_by_cwe(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Aggregate findings by CWE ID with confidence scores.
        
        Args:
            findings: List of vulnerability findings
            
        Returns:
            Dictionary mapping CWE IDs to aggregated information
        """
        cwe_aggregates = {}
        ...
        for finding in findings:
            ...
            if cwe_id not in cwe_aggregates:
                cwe_aggregates[cwe_id] = {
                    'count': 0,
                    'max_confidence': 0.0,
                    'avg_confidence': 0.0,
                    'severities': [],
            ...
            cwe_aggregates[cwe_id]['count'] += 1
            cwe_aggregates[cwe_id]['max_confidence'] = max(
                cwe_aggregates[cwe_id]['max_confidence'],
                confidence
            )
            cwe_aggregates[cwe_id]['severities'].append(finding.get('severity', 'medium'))
            cwe_aggregates[cwe_id]['findings'].append(finding)
        
        # Compute average confidence for each CWE
        for cwe_id, data in cwe_aggregates.items():
            confidences = []
            ...
            data['avg_confidence'] = round(sum(confidences) / len(confidences), 3) if confidences else 0.0
        
        return cwe_aggregates
