...
class HybridVulnerabilityScanner:
    ...
    def scan_file(self, file_path: str, mode: str = "hybrid", ml_confidence: float = 0.5) -> Dict:
        """
        ...
        if mode in ['ml', 'hybrid'] and self.ml_detector:
            ml_vulnerable, ml_confidence, ml_explanation = self.ml_detector.predict_vulnerability(content, ml_confidence)
            results['ml_analysis'] = {
    ...
    def _calculate_hybrid_score(self, results: Dict) -> Dict:
        ...
        ml_score = results['ml_analysis']['confidence'] if results['ml_analysis'] else 0.0
