...
"""
GraphCodeBERT Tokenization Pipeline for Vulnerability Detection
================================================================
Production-ready tokenization script with chunked batch processing and validation.

Rewards:
✅ Successful tokenization with proper shapes
✅ Correct label conversion and balance check
✅ Efficient chunked batch processing (memory-safe)
...
class TokenizationConfig:
    """Configuration for GraphCodeBERT tokenization pipeline"""
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("🚀 GraphCodeBERT Tokenization Pipeline - Enhanced Production Mode")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info(f"   ├── train_tokenized_graphcodebert.pt ({len(train_data)} samples)")
        logger.info(f"   ├── val_tokenized_graphcodebert.pt ({len(val_data)} samples)")
        logger.info(f"   ├── test_tokenized_graphcodebert.pt ({len(test_data)} samples)")
        if config.use_cache:
