...
- Test F1: ~0.60 ± 0.05 (aligned with GraphCodeBERT paper)
...
Input Path:
/kaggle/input/codeguardian-dataset-for-model-fine-tuning/tokenized/graphcodebert-base/{split}_tokenized_graphcodebert-base.pt
...
3. Run: python train_graphcodebert_lora_vFinalStable.py
4. Monitor training progress in console output
5. Collect artifacts from /kaggle/working/lora_output_graphcodebert/
...
BASE_DIR = "/kaggle/input/codeguardian-dataset-for-model-fine-tuning/tokenized/graphcodebert-base"
OUTPUT_DIR = "/kaggle/working/lora_output_graphcodebert"
...
def main():
    ...
    train_dataset = PreTokenizedDataset(f"{BASE_DIR}/train_tokenized_graphcodebert-base.pt")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_metrics['f1'] > best_val_f1 + EARLY_STOP_MIN_DELTA:
            ...
            checkpoint_path = f"{OUTPUT_DIR}/checkpoints_graphcodebert/epoch_graphcodebert_{epoch}.pt"
            torch.save({
                'epoch': epoch,
    ...
    best_checkpoint = torch.load(
        f"{OUTPUT_DIR}/checkpoints_graphcodebert/epoch_graphcodebert_{epoch - patience_counter}.pt"
    )
    ...
    with open(f"{OUTPUT_DIR}/metrics_graphcodebert/results_graphcodebert.json", 'w') as f:
        json.dump(results, f, indent=2)

    with open(f"{OUTPUT_DIR}/metrics_graphcodebert/language_wise_f1_graphcodebert.json", 'w') as f:
    ...
    with open(f"{OUTPUT_DIR}/metrics_graphcodebert/confusion_matrix_graphcodebert.json", 'w') as f:
        json.dump(cm_data, f, indent=2)

    print(f"\n💾 Saved to metrics_graphcodebert/language_wise_f1_graphcodebert.json")
    print(f"💾 Saved confusion matrix to metrics_graphcodebert/confusion_matrix_graphcodebert.json")

    # Save README
    readme_content = f"""# codeGuardian - GraphCodeBERT LoRA Adapter
...
"""
    ...
    with open(f"{OUTPUT_DIR}/README_graphcodebert.md", 'w') as f:
