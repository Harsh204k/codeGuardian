...
class Config:
    ...
    CHECKPOINT_DIR = f"{OUTPUT_DIR}/checkpoints_codebert"
    METRICS_DIR = f"{OUTPUT_DIR}/metrics_codebert"
    LOG_DIR = f"{OUTPUT_DIR}/logs"
    FINAL_MODEL_DIR = f"{OUTPUT_DIR}/codebert_lora_final"
    MERGED_MODEL_DIR = f"{OUTPUT_DIR}/codebert_lora_merged"
...
def initialize_model(config: Config, logger: logging.Logger) -> nn.Module:
    ...
    model = CodeBERTClassifier(config.MODEL_NAME, config.NUM_LABELS)
...
def train(config: Config, logger: logging.Logger):
    ...
    logger.info("=" * 70)
    logger.info("CODEBERT LORA FINE-TUNING (PURE-CODE)")
    logger.info("=" * 70)
    ...
    train_data = torch.load(os.path.join(config.DATA_PATH, "train_tokenized_codebert-base.pt"))
    ...
    try:
        test_data = torch.load(os.path.join(config.DATA_PATH, "test_tokenized_codebert-base.pt"), map_location="cpu")
        if "language" in test_data:
            ...
            logger.info("✓ Saved language-wise metrics to metrics_codebert/language_wise_f1.json")
        else:
    ...
    os.system("cp -r /kaggle/working/metrics_codebert /kaggle/working/metrics_output_codebert || true")
    os.system("cp -r /kaggle/working/checkpoints_codebert /kaggle/working/checkpoints_output_codebert || true")
    os.system("cp -r /kaggle/working/codebert_lora_merged /kaggle/working/codebert_lora_merged || true")
    ...
    try:
        ...
        trainer_export_dir = f"{config.OUTPUT_DIR}/codebert_trainer_export"
    ...
    try:
        ...
        repo_id = "urva-gandhi/codeGuardian-CodeBERT"
        ...
        model_hf.push_to_hub(repo_id, commit_message="Upload CodeGuardian CodeBERT-LoRA v1")
        tokenizer_hf.push_to_hub(repo_id)

        logger.info("✅ Model successfully pushed to Hugging Face Hub!")
        logger.info("🔗 https://huggingface.co/urva-gandhi/codeGuardian-CodeBERT")
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune CodeBERT with LoRA")
