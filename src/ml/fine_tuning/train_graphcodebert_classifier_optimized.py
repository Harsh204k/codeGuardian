# type: ignore
"""
Ultra-Fast GraphCodeBERT Fine-Tuning (FP16 Embeddings + Linear Classifier)
Kaggle-Ready: 50x faster than full model training, handles 500k+ samples

Stage 1: Extract frozen GraphCodeBERT embeddings ([CLS] token) using FP16 + chunked saving
...
EMBEDDINGS_DIR = "/kaggle/working/embeddings/graphcodebert"
...
# ========================================
print("\n" + "=" * 70)
print("STAGE 1: Extracting Frozen GraphCodeBERT Embeddings (FP16 + Chunked)")
print("=" * 70)
...
if embeddings_exist:
    ...
    train_data = torch.load(f"{KAGGLE_INPUT_PATH}/train_tokenized_graphcodebert.pt")
    val_data = torch.load(f"{KAGGLE_INPUT_PATH}/val_tokenized_graphcodebert.pt")
    test_data = torch.load(f"{KAGGLE_INPUT_PATH}/test_tokenized_graphcodebert.pt")
    ...
    print(f"\n🔧 Loading frozen GraphCodeBERT backbone...")
...
print(f"   (vs 124M+ for full GraphCodeBERT model - 80,000x fewer params!)")
