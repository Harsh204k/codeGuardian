import pandas as pd
from src.engine.risk_scorer import (
...
def export_excel(findings, out_path, cve_findings=None):
            ...
            rows.append(
                {
                    ...
                    "Name of Application Tested": (
                        findings[0].app if findings else "AppUnderTest"
                    ),
                    "Language": "dependency",
    ...
    print(
        f"Project Risk Level: {project_risk['risk_level'] if 'project_risk' in locals() else 'UNKNOWN'}"
    )
    print(
        f"Total findings: {len(rows)}, High-risk: {project_risk['critical_count'] + project_risk['high_count'] if 'project_risk' in locals() else 'N/A'}"
    )
